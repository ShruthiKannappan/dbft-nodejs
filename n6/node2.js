...
class Node
{
    ...
    // for(let i=0; i<7 ;i++){
    //   if(this.delegateList.includes(i) && i!=this.nodeID ){
    //     let index = i;
    //     if(i>this.nodeID)
    //     {
    //       index--;
    //     }
    //     this.sendPrePrepare(this.sockets[index], block);
    //   }
    // }
  }
   ...
   broadcastPrepare(prepare) {
    ...
    // for(let i=0; i<7 ;i++){
    //   if(this.validatorList.includes(i) && i!=this.nodeID ){
    //     let index = i;
    //     if(i>this.nodeID)
    //     {
    //       index--;
    //     }
    //     this.sendPrepare(this.sockets[index], prepare);
    //   }
    // }
 ...
 sendNewView(socket, newView) {
  ...
  initiate_new_view(new_view , proofs){
    clearTimeout(this.speaker_timer[this.nodeID]);
    ...
    //  await new Promise(resolve => setTimeout(resolve, this.timeout_duration /10));
  ...
  // clearTimer(sequenceNumber) {
  //   const timer = this.message_timers.get(sequenceNumber);
  //   if (timer) {
  //     clearTimeout(timer);
  //     this.message_timers.delete(sequenceNumber);
  //   }
  // }

  // clearAllTimers() {
  //   this.message_timers.forEach((timer, sequenceNumber) => {
  //     clearTimeout(timer);
  //     this.message_timers.delete(sequenceNumber);
  //   });
  //   console.log("cleared all timers");
  //   this.message_timers.clear();
  // }

  // startTimer(sequenceNumber) {
  //   this.clearTimer(sequenceNumber); // Clear any existing timer for the sequence number
  //   let new_view_propose = this.current_view_number+1;
  //   const timer = setTimeout(() => {
  //     if(this.current_view_number+1==new_view_propose){
  //     console.log(`Timer expired for sequence number: ${sequenceNumber}`);
  //     // Handling timeout here
  //     this.initiate_view_change(this.current_view_number);
  //     this.clearAllTimers(); }
  //   }, this.timeout_duration);
  //   this.message_timers.set(sequenceNumber, timer);
  //   console.log(`Timer started for sequence number: ${sequenceNumber}`);
  // }

// async speaker()
// {
  // if( this.nodeID == this.current_view_number){
    // if (this.own_message_pool.size > 0) {
        // let messagesIterator = this.own_message_pool.values();
        // let message = messagesIterator.next().value;
        // this.own_message_pool.delete(message);
        // this.started_message_pool.add(message);
        // // Increase the sequence number for the new message
        

        // // Broadcast a preprepare message
        // this.current_sequence_number++;
        // if(!this.viewchanged){this.initiate_new_view(this.current_view_number,this.viewChange_pool[this.current_view_number]);this.viewchanged = true;}
        // delete this.viewChange_pool[this.nodeID];

        // clearTimeout(this.speaker_timer);
        // this.initiate_new_view
        // this.initiate_preprepare(this.current_sequence_number, message, this.current_view_number);
  ...
  //   if(this.current_view_number+1==new_view_propose ){
  //     console.log(`Timer expired for sequence number for speaker delegate: ${this.current_sequence_number}`);
  //     // Handling timeout here
  //   this.initiate_view_change(this.current_view_number);
  //     this.clearAllTimers(); 
  //     // this.raisedViewChange = true;
  ...
  //   this.speaker_timer = setTimeout(() => {
  //     if(this.current_view_number+1==new_view_propose){
  //     console.log(`Timer expired for sequence number validator: ${this.current_sequence_number}`);
  //     // Handling timeout here
  //   this.initiate_view_change(this.view_number);}
  //     this.clearAllTimers(); 
 ...
 initiate_commit(sequence_number,message, view_number)
  ...
  committed(message,sequence_number,view_number)
  {
    ...
    //  this.started_message_pool.delete(message);
    //  this.own_message_pool.delete(message);
  ...
  decideResult(){
    ...
    this.vote_pool = [];
    this.current_view_number = 0;
    // this.speaker_timer[0] = [null,null,null];
    this.viewChange_pool[0] = [null,null,null];
        ...
        case MESSAGE_TYPE.pre_prepare:
            ...
            if(this.delegateList.includes(this.nodeID) && this.current_view_number ==data.preprepare.view_number && this.current_view_number == data.preprepare.nodeID  && ((!this.message_map.hasOwnProperty(data.preprepare.sequence_number))||data.preprepare.nodeID == this.nodeID))
            {
                ...
                let preprepare = data.preprepare;
                    ...
                    this.message_map[preprepare.sequence_number] = preprepare.message;
                    // this.startTimer(preprepare.sequence_number);
                ...
                if(this.message_map.hasOwnProperty(commit.sequence_number))
                {
                    if(this.commit_pool.hasOwnProperty(commit.sequence_number))
                    {
                       let  message_present = false;
                        ...
                        if(this.commit_pool[commit.sequence_number].length >= 3  && !this.committed_local.has(commit.sequence_number))// 2f+1
                        {
                            // this.clearTimer(commit.sequence_number);
